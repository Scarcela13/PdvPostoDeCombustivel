...
public class AcessoController {
    ...
    @Operation(summary = "Criar primeiro administrador", description = "Cria o primeiro administrador do sistema")
    @ApiResponses(value = {
        @ApiResponse(responseCode = "201", description = "Administrador criado com sucesso",
            content = @Content(schema = @Schema(implementation = AcessoResponse.class))),
        @ApiResponse(responseCode = "400", description = "Já existe um administrador")
    })
    public ResponseEntity<AcessoResponse> criarPrimeiroAdmin(@Valid @RequestBody AcessoRequest request) {
        AcessoResponse response = acessoService.criar(request);
        return ResponseEntity.status(HttpStatus.CREATED).body(response);
    }
}
